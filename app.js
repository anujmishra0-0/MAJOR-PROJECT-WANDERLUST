...
const dbUrl = process.env.ATLASDB_URL || localDbUrl;
const SECRET = process.env.SECRET || "thisshouldbeabettersecret";
const port = process.env.PORT || 8080;

if (process.env.NODE_ENV === "production" && !process.env.ATLASDB_URL) {
    console.error("ERROR: ATLASDB_URL must be set in production.");
    process.exit(1);
}
...
async function main(){
    try {
        if (dbUrl !== localDbUrl) {
            ...
            console.log("Using public DNS servers for Atlas SRV lookup: 8.8.8.8, 8.8.4.4");
        }
